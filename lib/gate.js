...
export async function decide({ ctx, engine, policy, agent, session, sessionId, trigger, config, logger }) {
  ...
  // Policy: pruning never lands on its own.
  //
  // Pruning abbreviates tool results in place, so the surface stays almost as
  // large as before and the cache break is paid on the next request at full
  // size — measured at 724k cold tokens for a prune that freed 93k. A
  // summarization in the same operation pays that break once and leaves a much
  // smaller surface, so the prune's own damage is subsumed and its abbreviation
  // even shrinks the summarizer's input.
  //
  // A plan whose pruning alone would get below the threshold is therefore
  // deferred silently: nothing happens, the surface keeps growing, and the guard
  // asks once the pressure has risen far enough that a summarization follows in
  // the same operation. The user's decision is then worth making.
  if (plan.kind === 'prune-only') {
    logger.info(
      `cache-guard: deferred a prune-only pass (${plan.pruned.length} tool results, `
      + `${plan.afterPruneTokens} tokens would stay above the ${plan.thresholdTokens} threshold); `
      + 'pruning lands only together with a summarization',
    )
    return 'declined'
  }
