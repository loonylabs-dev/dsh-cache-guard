 ...
 *
 * @param input the ctx, the agent and session to operate on, the plan, the signal, and a logger.
 * @returns the engine's own compaction result, for the calling listener to log.
 */
export async function runOperation({ ctx, agent, session, plan, signal, logger }) {
  const summary = plan.summary
  if (summary === null) return null
  const result = await ctx.compaction.compactRegion(summary.startSeq, summary.endSeq, agent, signal)
  const pruner = ctx.get('toolResultPruner')
  if (pruner !== undefined) {
    try {
...
export function installGate({ ctx, engine, policy, config, logger }) {
  ...
  engine.compactIfNeeded = async function gated(agent, trigger, signal) {
    ...
    return runOperation({ ctx, agent, session, plan, signal, logger })
  }
  return () => {
