 ...
 *
 *   GET  /cache-guard/state?session=<id>  -> mode, default mode, price, reach, last plan
...
export function statePayload({ store, config, sessionId }) {
  return {
    ...
    /**
     * Compaction engines this process guards. `0` means no automatic rewrite can
     * be intercepted here, which the client half must not present as protection.
     */
    engines: store.engines?.() ?? 0,
    /** Latest pressure reading; changes every step. It exists only after a decision. */
